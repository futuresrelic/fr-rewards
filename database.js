...
function initializeTables() {
  ...
  // Migration: Create module_instances table for database-backed module system
  try {
    const moduleInstancesExists = db.prepare(`
      SELECT name FROM sqlite_master WHERE type='table' AND name='module_instances'
    `).get();

    if (!moduleInstancesExists) {
      console.log('🔄 Creating module_instances table...');
      ...
      db.exec(`
        CREATE INDEX IF NOT EXISTS idx_module_instances_created_by
        ON module_instances(created_by);
      `);

      console.log('✅ module_instances table created');
    }
  } catch (error) {
    console.warn('⚠️ Migration warning:', error.message);
  }
...
// Module Instances methods for database-backed module system
const moduleInstances = {
  /**
   * Create a new module instance
   * @param {Object} data - Module instance data
   * @param {string} data.id - Unique identifier (generated by caller, e.g., 'mod_abc123')
   * @param {string} data.module_type - Type of module (e.g., 'claim-rewards', 'factory-craft')
   * @param {string} data.name - Descriptive name for admin reference
   * @param {string} data.description - Optional description
   * @param {Object} data.config - Module configuration object
   * @param {string} data.created_by - Wallet account of creator
   * @returns {Object} Result of the insert operation
   */
  create: (data) => {
    const stmt = db.prepare(`
      INSERT INTO module_instances (id, module_type, name, description, config, created_by)
      VALUES (?, ?, ?, ?, ?, ?)
    `);
   ...
   * @returns {Object|null} Module instance with parsed config, or null if not found
   */
  get: (id) => {
    const instance = db.prepare('SELECT * FROM module_instances WHERE id = ?').get(id);
   ...
   * @param {string} module_type - Optional module type filter
   * @returns {Array} Array of module instances with parsed configs
   */
  getAll: (module_type = null) => {
    let query = 'SELECT * FROM module_instances';
    let params = [];
   ...
   */
  delete: (id) => {
    return db.prepare('DELETE FROM module_instances WHERE id = ?').run(id);
  },
   ...
   */
  exists: (id) => {
    const result = db.prepare('SELECT id FROM module_instances WHERE id = ?').get(id);
    return !!result;
  },

  /**
   * Get module instances created by a specific user
   * @param {string} wallet_account - Creator's wallet account
   * @returns {Array} Array of module instances
   */
  getByCreator: (wallet_account) => {
