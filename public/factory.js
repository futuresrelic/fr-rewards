...
document.getElementById('retry-failed-btn').addEventListener('click', async () => {
  ...
  if (timeSinceLastRetry < cooldownMs) {
    ...
    alert(`Please wait ${remainingSeconds} seconds before retrying again.`);
    return;
  }

  if (!confirm('This will attempt to mint results for any failed crafts where assets were successfully transferred. Continue?')) {
    return;
  }

  showProcessingModal('Retrying Failed Crafts', 'Searching for failed crafts and verifying transfers...');

  try {
    const response = await fetch(`${API_URL}/api/factory/retry-failed`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ user_wallet: currentAccount })
    });
    ...
    // Show results
    if (data.retried === 0) {
      showProcessingModal('No Failed Crafts', `
        <p style="color: var(--text-secondary);">No failed crafts found that can be retried.</p>
        <p style="font-size: 0.9rem; margin-top: 10px;">Total failed crafts checked: ${data.total_failed || 0}</p>
        <button class="btn btn-primary" onclick="hideProcessingModal()">Close</button>
      `);
    } else {
      const successList = data.results
        .filter(r => r.success)
        .map(r => `<li>${r.recipe_name} - <a href="https://waxblock.io/transaction/${r.mint_transaction_id}" target="_blank" style="color: var(--primary);">View TX</a></li>`)
        .join('');

      const failedList = data.results
        .filter(r => !r.success)
        .map(r => `<li>${r.recipe_name || 'Unknown'} - ${r.error}</li>`)
        .join('');

      showProcessingModal('✅ Retry Complete!', `
        <p style="color: #4ade80; margin-bottom: 15px;">Successfully retried ${data.retried} out of ${data.total_failed} failed craft(s)!</p>
        ${successList ? `
          <div style="margin: 15px 0; padding: 15px; background: var(--bg-dark); border-radius: 6px; text-align: left;">
            <strong style="color: #4ade80;">✅ Successful:</strong>
            <ul style="margin: 10px 0; padding-left: 20px;">${successList}</ul>
          </div>
        ` : ''}
        ${failedList ? `
          <div style="margin: 15px 0; padding: 15px; background: var(--bg-dark); border-radius: 6px; text-align: left;">
            <strong style="color: #f87171;">❌ Still Failed:</strong>
            <ul style="margin: 10px 0; padding-left: 20px;">${failedList}</ul>
          </div>
        ` : ''}
